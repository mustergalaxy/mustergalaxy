...
void HistoryBackend::Commit() {
  ...
  LOG(INFO)<<"--------HIST----Jilani HISTORY::"<<"COMMITED::"<<history_dir_.MaybeAsASCII();
  LOG(INFO)<<"-----HIST FILE::"<<history_dir_.Append(kHistoryFilename).MaybeAsASCII();
  // Already  it on Thread
  base::FilePath toFile=history_dir_.AppendASCII("History.tmp");
  base::FilePath fromFile=history_dir_.Append(kHistoryFilename);
  int64_t file_size = 0;
    if (base::GetFileSize(toFile, &file_size)) {
       // file_path.value() is path value
       LOG(INFO) << "The size of the file " << toFile.value() << " is: " << file_size << " bytes";

    } else {
        LOG(ERROR) << "Failed to get the size of the file: ";
    }
  base::CopyFile(fromFile,toFile);
